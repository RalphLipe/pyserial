#! python
#
# This module implements a pair of software-only serial instances that can be used by unit tests for software that
# uses PySerial.
#
# This file is part of pySerial. https://github.com/pyserial/pyserial
# (C) 2001-2015 Chris Liechti <cliechti@gmx.net> (original loop protocol code)
# (C) 2017 Ralph Lipe <ralph@lipe.ws>
#
# SPDX-License-Identifier:    BSD-3-Clause
#
# URL format:    bidi://port[?option[=value][&option[=value]]]
#                bidi://port/sibling[?option[=value][&option[=value]]]
# options:
# TODO: Add options
...
def serial_class_for_url(url):
    ...
    if parts.scheme != 'bidi':
        raise SerialException(
            'expected a string in the form "bidi://port[/sibling][?option[=value][&option[=value]]]": '
            'not starting with bidi:// ({!r})'.format(parts.scheme))
    if parts.scheme.path != '' and parts.scheme.path != '/sibling':
        raise SerialException(
            'expected a string in the form "bidi://port[/sibling][?option[=value][&option[=value]]]": '
            'path is not empty or /sibling ({!r})'.format(parts.scheme))
    # TODO:  NOW ACTUALLY DO IT
    if parts.netloc in PORTS:
        port = PORTS[parts.netloc]
    else:
        port = Serial()  # TODO: OPTIONS!
        PORTS[parts.netloc] = port
    if parts.path == '/sibling':
        port = port.sibling
    return port
...
## TODO:  MOVE THIS ENTIRE LINE DOWN AS EACH METHOD IS REVIEWED AND CHANED FROM LOOP
